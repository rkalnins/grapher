...
MainController::MainController ( QObject *parent ) : QObject { parent } {
    connect(&menu_controller_, &MenuController::saveWorkspaceAsClicked,
            this,
    ...
    connect(&menu_controller_, &MenuController::exportDataClicked, this,
            &MainController::handleCsvExport);

    connect(&workspace_controller_, &WorkspaceController::settingUpdated,
            this,
...
void MainController::setWorkspaceModel ( models::WorkspaceModel &model ) {
    ...
    workspace_created_connection_ = connect(workspace_model_,
...
void MainController::configureProviders () {
    ...
    qDebug() << "connecting handlers and channels "
             << data_model_->getHandlerCount();
    ...
    for ( int i = 0; i < data_model_->getHandlerCount(); ++i ) {
        ...
        AbstractDataProviderModel *provider = providers_->getProviderByName(
                p);
        if ( provider ) {
            handler->setChannel(provider->getChannelById(id));
            qDebug() << "connected handler to channel "
                     << handler->getName() << " -> "
...
void
MainController::setProvidersModel ( models::DataProvidersModel &model ) {
    providers_ = &model;
}

void MainController::setMenuModel ( models::MenuModel &model ) {
    if ( menu_model_ ) {
        disconnect(menu_model_, &MenuModel::titleChanged, this,
                   &MainController::handleTitleChanged);
    }
    menu_model_ = &model;
    menu_controller_.setModel(model);
    connect(menu_model_, &MenuModel::titleChanged, this,
            &MainController::handleTitleChanged);
...
void MainController::connectSaveSignals () {
    ...
    connect(data_model_, &DataModel::collectedGraphData,
            workspace_model_->getWorkspace(),
            &WorkspaceHandler::saveWorkspaceGraphs);
    connect(workspace_model_->getWorkspace(),
            &WorkspaceHandler::needSaveWorkspaceGraphs, data_model_,
...
void MainController::disconnectSaveSignals () {
    ...
    disconnect(data_model_, &DataModel::collectedGraphData,
               workspace_model_->getWorkspace(),
               &WorkspaceHandler::saveWorkspaceGraphs);
    disconnect(workspace_model_->getWorkspace(),
               &WorkspaceHandler::needSaveWorkspaceGraphs, data_model_,
               &DataModel::collectSaveGraphsData);
}
...
void MainController::handleCsvExport ( const QUrl &url,
