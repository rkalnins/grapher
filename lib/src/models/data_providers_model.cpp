...
void
DataProvidersModel::populateProviders ( const QJsonArray &providers ) {
    reset();
    qDebug() << providers;
...
bool DataProvidersModel::setData ( const QModelIndex &index,
                                   const QVariant &value, int role ) {
    return QAbstractItemModel::setData(index, value, role);
}

QVariant
DataProvidersModel::data ( const QModelIndex &index, int role ) const {
...
const QList< AbstractDataProviderModel * > &
DataProvidersModel::getProviders () const {
    return providers_;
}

AbstractDataProviderModel *
DataProvidersModel::getProviderByName ( const QString &name ) {
